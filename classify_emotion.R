classify_emotion <- function(textColumns,
	...
	lexicon <- read.csv("data/emotions.csv", header = FALSE)
    ...
    for (i in 1:nrow(matrix)) {
        ...
        scores <- list(anger = 0, disgust = 0, fear = 0, 
        ...
        if (best_fit == "disgust" && 
            as.numeric(unlist(scores[2])) - 3.09234 < .01) {
            best_fit <- NA
        }
		
        documents <- 
            rbind(documents,
                  c(scores$anger, scores$disgust, scores$fear, 
                    scores$joy, scores$sadness, scores$surprise, best_fit))
	}
