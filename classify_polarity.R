classify_polarity <- function(textColumns,
    ...
    counts <- 
        list(positive = length(which(lexicon[, 3] == "positive")),
    ...
    for (i in 1:nrow(matrix)) {
		...
		documents <- 
		    rbind(documents,
		          c(scores$positive, scores$negative, 
		            abs(scores$positive / scores$negative), best_fit, negation))
		
		if (verbose) {
			print(paste("POS:", scores$positive,
			            "NEG:", scores$negative,
			            "RATIO:", abs(scores$positive/scores$negative)))
			cat("\n")
		}
	...
	write.table(documents, 
	            file = "check_polarity.csv", 
	            append = FALSE, quote = FALSE, sep = ";",
	            eol = "\n", na = "NA")
	
	print(paste("Positivos: ", compos,
	            "Negativos: ", comneg,
	            "Neutrales: ", comneu))
	cat("\n")
	sink()
	...
	colnames(documents) <- 
	    c("POSITIVO", "NEGATIVO", "POSITIVO/NEGATIVO", 
	      "BEST_FIT", "NEGATION WORDS")
